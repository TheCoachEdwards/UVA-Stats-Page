...
All_Team_Names <- read.csv("~/Desktop/New Team Names.csv")
...
FBSOff <- FBSOff %>% filter(no_play == 0)
FBSDef = subset(NoKicks, defense %in% FBSSchools)
FBSDef <- FBSDef %>% filter(no_play == 0)
...
## # of Pass Blockers
```{r, echo= FALSE, warning=FALSE, message = FALSE}
PassBlockersOff <- FBSOff %>% filter(run_pass == "P" & run_pass_option == 0) %>% group_by(OffTeamName, pass_blocking_number) %>% summarize(OffPassBlockPlays = n(), OffPassBlockEfficiency = sum(Efficient, na.rm = TRUE), OffPassBlockEfficiencyPerc = mean(Efficient, na.rm = TRUE), OffPassBlockYPP = mean(Gain.Loss, na.rm = TRUE)) %>% rename(TeamName = OffTeamName)
PassBlockersDef <- FBSDef %>% filter(run_pass == "P" & run_pass_option == 0) %>% group_by(DefTeamName, pass_blocking_number) %>% summarize(DefPassBlockPlays = n(), DefPassBlockEfficiency = sum(Efficient, na.rm = TRUE), DefPassBlockEfficiencyPerc = mean(Efficient, na.rm = TRUE), DefPassBlockYPP = mean(Gain.Loss, na.rm = TRUE)) %>% rename(TeamName = DefTeamName)
...
## Pass Pro Efficiency
```{r, echo= FALSE, warning=FALSE, message = FALSE}
PassBlockersOff <- FBSOff %>% filter(run_pass == "P" & run_pass_option == 0) %>% group_by(OffTeamName, pass_blocking_number) %>% summarize(OffPassBlockPlays = n(), OffPassBlockEfficiency = sum(!is.na(pass_rush_result)), OffPassBlockEfficiencyPerc = (OffPassBlockEfficiency / OffPassBlockPlays)) %>% rename(TeamName = OffTeamName)
PassBlockersDef <- FBSDef %>% filter(run_pass == "P" & run_pass_option == 0) %>% group_by(DefTeamName, pass_blocking_number) %>% summarize(DefPassBlockPlays = n(), DefPassBlockEfficiency = sum(!is.na(pass_rush_result)), DefPassBlockEfficiencyPerc = (DefPassBlockEfficiency / DefPassBlockPlays)) %>% rename(TeamName = DefTeamName)
...
tbl <- reactable(PassBlockersCombined,
                 columns = list(
                   ...
                   OffPassBlockPlays = colDef(defaultSortOrder = "desc", align = "center",width = 75, name = "Plays"),
                   OffPassBlockEfficiency = colDef(defaultSortOrder = "desc", align = "center",width = 75, name = "Pressure Allowed"),
                   
                   OffPassBlockEfficiencyPerc = colDef(defaultSortOrder = "desc", width = 75, align = "center", name = "Pass Pro %", format = colFormat(percent = TRUE, digits = 1), 
                                                             footer = function(values, digits = 1) {
                                                               htmltools::tags$b(sprintf("%.1f", mean(values, na.rm = TRUE)))
                                                             },
                                              style = function(value) {
                   ...
                   DefPassBlockPlays = colDef(defaultSortOrder = "desc", align = "center", width = 75, name = "Plays"),
                   DefPassBlockEfficiency = colDef(align = "center", width = 75, name = "Pressure Allowed"),
                   
                   DefPassBlockEfficiencyPerc = colDef(width = 75, align = "center", name = "Pass Pro %", format = colFormat(percent = TRUE, digits = 1), 
                                                             footer = function(values, digits = 1) {
                                                               htmltools::tags$b(sprintf("%.1f", mean(values, na.rm = TRUE)))
                                                             },
                                                             style = function(value) {
                                               ...
                                               list(background = color)})),
                 columnGroups = list(
                   colGroup(name = "Offense", columns = c("OffPassBlockPlays", "OffPassBlockEfficiency", "OffPassBlockEfficiencyPerc")),
                   colGroup(name = "Defense", columns = c("DefPassBlockPlays", "DefPassBlockEfficiency", "DefPassBlockEfficiencyPerc"))),
                 bordered = TRUE,
                 highlight = TRUE,
                 searchable = TRUE,
                 showPageSizeOptions = TRUE, 
